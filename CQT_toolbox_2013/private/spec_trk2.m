function[SPitch, VUVSPitch, pAvg, pStd,m2,m3,m4]= spec_trk(Data, Fs, VUVEnergy, Prm)
...
%         pause
%         subplot(3,1,1)
%         t = linspace(0,length(Signal)/Fs*1000,length(Signal));
%         plot(t,Signal)
%         subplot(3,1,2)
% %         lenf = length(Magnit1);
% %         fre = linspace(0,lenf/(8192)*20000,lenf);
%         plot(Magnit1)
%         subplot(3,1,3)
% %         lenf2 = length(SHC);
% %         fre2 = linspace(0,lenf2/(8192)*20000,lenf2);
%         plot(SHC)
        a = max(SHC);
%         display(a);
%         pause
...
pAvg = mean(VPitch);
pStd = std(VPitch);
% m1 = moment(VPitch,1); %first order moment
m2 = moment(VPitch,2); %second order moment
m3 = moment(VPitch,3); %third order moment
m4 = moment(VPitch,3); %fourth order moment
% m5 = moment(VPitch,3); %fifth order moment
% m6 = moment(VPitch,3); %sixth order moment
